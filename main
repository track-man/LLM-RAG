...
def init_system() -> None:
    ...
    # ====================== 步骤1：加载原始文档 ======================
    logger.info(f"加载原始文档（路径：{config.RAW_DOC_DIR}）...")
    # 调用数据处理模块：src/data_processing/document_loader.py
    # 输入规范：
    #   doc_dir: 原始文档目录路径（从config读取）
    # 输出规范：
    #   文档列表，每个元素为{"text": 文本内容, "metadata": {"source": 来源路径}}
    docs = load_documents(config.RAW_DOC_DIR)
    if not docs:
        raise ValueError(f"未在{config.RAW_DOC_DIR}找到任何文档，请检查路径或添加文档")
    logger.info(f"原始文档加载完成，共{len(docs)}个文档")

    # ====================== 步骤2：文本分块 ======================
    logger.info(f"文本分块（大小：{config.CHUNK_SIZE}，重叠：{config.CHUNK_OVERLAP}）...")
    ...
    # ====================== 步骤3：生成嵌入向量 ======================
    logger.info(f"生成嵌入向量（模型：{config.EMBEDDING_MODEL}）...")
    # 提取分块文本
    ...
    # ====================== 步骤4：构建Chroma向量库 ======================
    logger.info(f"将分块与嵌入存入向量库（路径：{config.CHROMA_PATH}）...")
    # 调用数据处理模块：src/data_processing/embedding_handler.py
    # 输入规范：
    #   chunks: 分块列表（同split_text输出）
    #   embeddings: 嵌入向量列表（同generate_embeddings输出）
    #   chroma_path: Chroma向量库存储路径（从config读取）
    # 输出规范：无（向量库持久化到本地）
    index_documents(chunks, embeddings, config.CHROMA_PATH)
...
def interactive_query() -> None:
    ...
    while True:
        ...
        # 输入规范：
        #   query: 用户查询字符串
        #   chroma_path: Chroma向量库路径（从config读取）
